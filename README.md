...
新安装只在本机 HTTP 和数据库检查通过后报告完成；失败时显示日志。非交互安装须设置 `PORT`，例如 `PORT=18080 IPVER=6 NONINTERACTIVE=1 sh install.sh`。非交互环境下已装过的机器默认走保留数据修复，加 `FORCE_INSTALL=1` 可强制重新安装。NAT 入站信息可用 `PUBLIC_HOST` 和 `PUBLIC_PORT` 指定，仅用于显示访问地址，不会创建服务商端口映射。
...
- 安装与修复已合并为同一个一键命令：自动识别是否已安装，已安装时让你选保留数据修复（先备份、检查失败自动恢复）还是重新安装（可换端口）；非交互环境默认修复，`FORCE_INSTALL=1` 可强制重装。
