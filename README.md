...
- **多用户**：登录没有用户名，只输密码——不同的密码就是不同的账号。账号没有注册入口，只能由管理员在控制台\"用户管理\"里添加。普通用户可以查看所有文件、发文件、建接收链接，但不能删除任何文件，只能取消自己的分享链接；管理员可以删除任何用户的文件、取消任何用户的分享链接。老版本升级上来时，原来的密码自动成为管理员账号。登录有防暴力破解：同一 IP 10 分钟内输错 20 次密码会被拦 10 分钟，登录成功清零。
