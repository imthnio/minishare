...
for u in "https://raw.githubusercontent.com/imthnio/wenjianchuanshu/main/install.sh" "https://cdn.jsdelivr.net/gh/imthnio/wenjianchuanshu@main/install.sh"; do
  if command -v curl >/dev/null 2>&1; then
    curl -fSL --connect-timeout 15 --max-time 120 --retry 2 -o /tmp/minishare-install.sh "$u" 2>/dev/null && ok=1 && break
  elif command -v wget >/dev/null 2>&1; then
    wget -q -T 120 -O /tmp/minishare-install.sh "$u" 2>/dev/null && ok=1 && break
  fi
done
